...
# X = mat['sig_out']     # Distorted signal as input
# Y = mat['sig_in']       # Original signal as target

# Making the distortion indirection 
Y = mat['sig_out']     # Distorted signal as target
X = mat['sig_in']       # Original signal as input
...
# --- Evaluate model ---
with torch.no_grad():
    outputs = model(x_abs_seq_torch*10**(-2/20), x_theta_seq_torch)
...
# Convert outputs_np (real, imag) to a single complex-valued vector
outputs_complex = outputs_np[:, 0] + 1j * outputs_np[:, 1]
savemat("outputs_complex.mat", {"outputs_complex": outputs_complex})
##
