...
from fixed_PG_JANET_training_ILC import PGJanetRNN, max_norm

# --- Load and preprocess data (direct model: BB -> SISO) ---
mat_path = 'U_and_U_ideal_for_pjanet.mat'
mat = loadmat(mat_path, squeeze_me=True)
X = mat['u']  # Clean input
Y = mat['u_ideal']  # Distorted output
...
model.load_state_dict(torch.load("pg_janet_rnn_max_ILC.pth", map_location=device))
model.eval()
...
plt.legend()
plt.tight_layout()
plt.savefig('pg_janet_am_am_comparison_ILC.png')
plt.show()
...
plt.legend()
plt.tight_layout()
plt.savefig("pg_janet_am_pm_fixed_per_sequence_ILC.png")
plt.show()
...
plt.legend()
plt.tight_layout()
plt.savefig("pg_janet_spectrum_comparison_ILC.png")
plt.show()
