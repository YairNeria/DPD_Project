...
from train_pg_janet_class import PGJanetRNN, max_norm

# --- Load and preprocess data (invert=True for inverse model) ---
mat_path = 'U_and_U_ideal_for_pjanet.mat'
mat = loadmat(mat_path, squeeze_me=True)
X = mat['u_ideal']     # Distorted signal as input
Y = mat['u']       # Original signal as target
...
model.load_state_dict(torch.load("pg_janet_inverse_ILC.pth", map_location=device))
model.eval()
...
plt.legend()
plt.tight_layout()
plt.savefig('inverse_pg_janet_am_am_ILC.png')
plt.show()
...
plt.legend()
plt.tight_layout()
plt.savefig("inverse_pg_janet_am_pm_ILC.png")
plt.show()
...
def plot_spectrum(sig, label, color):
    ...
    spectrum_db = 20 * np.log10(np.abs(spectrum) + 1e-9) - 46.3
    spectrum_db_smooth = savgol_filter(spectrum_db, 201, 3)
    N = len(sig)
...
plt.legend()
plt.tight_layout()
plt.savefig("inverse_pg_janet_spectrum_comparison_ILC.png")
plt.show()
